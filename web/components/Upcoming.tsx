...
export default function Upcoming() {
  return (
    <section className="relative py-24 md:py-32 bg-[#0A1A36]">
      <div className="mx-auto max-w-container px-6 md:px-10">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          ...
          <Link href="/conference" className="text-sm font-display font-semibold text-gold-heritage hover:text-gold-bright transition-colors">
            See full schedule →
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {events.map((e, i) => (
            ...
            >
              <Link href={e.href} className="card group block p-8 md:p-10 relative overflow-hidden">
                <div className={`absolute top-0 left-0 h-1 w-full ${accentBar[e.accent]}`} />
                <div className="eyebrow text-mist/60 mb-4">{e.kind}</div>
                <h3 className="font-display font-bold text-white text-2xl md:text-3xl leading-tight mb-6">
                  {e.title}
                </h3>
                <div className="space-y-1 text-mist/70">
                  <div className="text-sm flex items-center gap-2">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" className="opacity-60">
                ...
                <div className="mt-8 text-sm font-display font-semibold text-white/80 group-hover:text-gold-heritage transition-colors">
                  Learn more →
                </div>
              </Link>
            </motion.div>
